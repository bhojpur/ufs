...
import (
	...
	idtools "github.com/bhojpur/kernel/pkg/idtools"
	timesys "github.com/bhojpur/time/pkg/system"
	"github.com/bhojpur/ufs/pkg/fileutils"
	"github.com/bhojpur/ufs/pkg/ioutils"
	"github.com/bhojpur/ufs/pkg/pools"
...
func gzDecompress(ctx context.Context, buf io.Reader) (io.ReadCloser, error) {
	if noPigzEnv := os.Getenv("BHOJPUR_DISABLE_PIGZ"); noPigzEnv != "" {
		noPigz, err := strconv.ParseBool(noPigzEnv)
		if err != nil {
			logrus.WithError(err).Warn("invalid value in BHOJPUR_DISABLE_PIGZ env var")
		}
		if noPigz {
			logrus.Debugf("Use of pigz is disabled due to BHOJPUR_DISABLE_PIGZ=%s", noPigzEnv)
...
func ReadSecurityXattrToTarHeader(path string, hdr *tar.Header) error {
	...
	capability, _ := common.Lgetxattr(path, "security.capability")
	if capability != nil {
...
func (ta *tarAppender) addTarFile(path, name string) error {
	...
	if !isOverlayWhiteout && !strings.HasPrefix(filepath.Base(hdr.Name), WhiteoutPrefix) && !ta.IdentityMapping.Empty() {
		...
		hdr.Uid, hdr.Gid, err = ta.IdentityMapping.ToLabni(fileIDPair)
		if err != nil {
			return err
		}
	}
	...
	if hdr.Typeflag == tar.TypeReg && hdr.Size > 0 {
		// We use filesys.OpenSequential to ensure we use sequential file
		// access on Windows to avoid depleting the standby list.
		// On Linux, this equates to a regular os.Open.
		file, err := filesys.OpenSequential(path)
		if err != nil {
			return err
		}
...
func createTarFile(path, extractDir string, hdr *tar.Header, reader io.Reader, Lchown bool, chownOpts *idtools.Identity, inUserns bool) error {
	...
	case tar.TypeReg, tar.TypeRegA:
		// Source is regular file. We use filesys.OpenFileSequential to use sequential
		// file access to avoid depleting the standby list on Windows.
		// On Linux, this equates to a regular os.OpenFile
		file, err := filesys.OpenFileSequential(path, os.O_CREATE|os.O_WRONLY, hdrInfo.Mode())
		if err != nil {
			return err
		}
	...
	for key, value := range hdr.Xattrs {
		if err := common.Lsetxattr(path, key, []byte(value), 0); err != nil {
	...
	// timesys.Chtimes doesn't support a NOFOLLOW flag atm
	if hdr.Typeflag == tar.TypeLink {
		if fi, err := os.Lstat(hdr.Linkname); err == nil && (fi.Mode()&os.ModeSymlink == 0) {
			if err := timesys.Chtimes(path, aTime, hdr.ModTime); err != nil {
				return err
			}
		}
		...
		if err := timesys.LUtimesNano(path, ts); err != nil && err != errsys.ErrNotSupportedPlatform {
			return err
		}
	}
	return nil
...
loop:
	...
	for _, hdr := range dirs {
		...
		if err := timesys.Chtimes(path, hdr.AccessTime, hdr.ModTime); err != nil {
			return err
		}
	}
	return nil
...
func (archiver *Archiver) CopyFileWithTar(src, dst string) (err error) {
	...
	if err := filesys.MkdirAll(filepath.Dir(dst), 0700); err != nil {
		return err
	}
